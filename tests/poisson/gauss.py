"""Tests for `flowx/poisson/gauss.py`."""
...
class TestPoissonGauss(unittest.TestCase):
    ...
    def test_number_of_iterations(self):
        ...
        maxiter, tol = 0, 1e-12
        ites, _ = flowx.poisson.solve_gauss(self.grid, 'ivar', 'rvar',
    ...
    def test_residual(self):
        ...
        ites, res = flowx.poisson.solve_gauss(self.grid, 'ivar', 'rvar',
                                                maxiter=maxiter, tol=tol)
    ...
    def test_error(self):
        ...
        ites1, res1 = flowx.poisson.solve_gauss(self.grid, 'ivar', 'rvar',
                                                  maxiter=maxiter, tol=tol)
        ...
        self.grid.set_values('ivar', numpy.zeros((self.grid.nx + 2,
                                                  self.grid.ny + 2)))
        maxiter, tol = 3000, 1e-6
        ites2, res2 = flowx.poisson.solve_gauss(self.grid, 'ivar', 'rvar',
                                                  maxiter=maxiter, tol=tol)
        self.grid.get_error('eror', 'ivar', 'asol')
