...
def ann_search(grid, particle, ibmf, options):
    ...
    for j in range(grid.ny + 2):
        for i in range(grid.nx + 2):
            ...
            tree_index = tree.get_nns_by_vector(
                [xmesh[j, i], ymesh[j, i]], nquery_trees
            )
            dist = 1e13

            trace_index, trace_distance = trace.get_nns_by_vector(
                [ymesh[j, i]], nquery_trace, include_distances=True
            )
            countit = 0

            niter_trace = numpy.size(
                numpy.where(numpy.array(trace_distance) < max_panel_length)
            )
