...
def solve_gauss_seidel(grid, ivar, rvar, maxiter=3000, tol=1e-9, verbose=False):
    ...
    -------
    ites: integer
        Number of the iterations computed.
    residual: float
        Final residual .
    ...
    b = grid.get_values(rvar) #I have already defined in the nbook what the rvar is by using the simulation.py.
    ...
    while ites < maxiter and residual > tol:
        ...
        phi[1:-1,1:-1] = ((phi[1:-1,:-2] + phi_old[1:-1,2:])*dy**2 + (phi[:-2,1:-1] + phi_old[2:,1:-1])*dx**2 - b[1:-1,1:-1]*dx**2*dy**2)/(2*(dx**2 + dy**2))
