...
class Quantum(object):
    def __init__(
        self, domain_data_struct=[None] * 5, quantum_vars=[None] * 2, quantum_info=None
    ):
        ...
        self._simulators = {
            "QASM": _interface.run_circuit_QASM,
            "IBMQ": _interface.run_circuit_IBMQ,
        }

        self._calibrators = {
            "QASM": _interface.calibrate_circuit_QASM,
            "IBMQ": _interface.calibrate_circuit_IBMQ,
        }

        # ----------------Read user parameters--------------------------------------------
        if quantum_info:
            for key in quantum_info:
                self._options[key] = quantum_info[key]

        # ----------------Setup current unit-----------------------------------------------

        if None in domain_data_struct:  # or None in quantum_vars:
            raise ValueError(
                "Quantum unit cannot be setup. One or more parameter is missing"
            )
            ...
            self.fitter, self.device, self.noise = self._calibrate_circuit(
                self.quantum_register,
            ...
            if self._options["circuit"] == "grover":
                self._gates = [
                    _interface.grover.oracle_gate,
                    _interface.grover.amplification_gate,
                ] * self._options["repeat"]
    ...
    def setup_circuit(self):
        ...
        for gate in self._gates:
            gate(self.circuit, self.quantum_register, self._particles, self._gridc)
    ...
    def run_circuit(self):
        ...
        results, answer = self._run_circuit(
            self.device,
            self.noise,
            self.fitter,
            self.circuit,
