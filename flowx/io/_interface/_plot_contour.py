...
def plot_contour(grid, ivar, auxvars=[], particle=None, scalars=None, levels=None):
    ...
    if not levels: levels = numpy.linspace(ivarmin,ivarmax,10) 
    ...
    for block in grid.blocklist:
        ...
        if len(levels) > 1:
            ...
            pyplot.contour(xmesh, ymesh, block[ivar][0,:,:], levels=levels, linestyles='solid', linewidths=1.0, colors='black')

        for var in auxvars:
            pyplot.contour(xmesh, ymesh, block[var][0,:,:], colors='blue', linestyles='solid', linewidths=1.0)
    ...
    if len(levels) > 1: pyplot.colorbar(label=ivar,format=ticker.FuncFormatter(_fmt))
    pyplot.axis('scaled')
