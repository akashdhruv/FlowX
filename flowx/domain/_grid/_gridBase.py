...
class GridBase(Dataset):
    ...
    def __init__(
        ...
        if nx % xblocks or ny % yblocks:
            raise ValueError(
                "[flowx.domain.GridBase]:(nx,ny) must be exactly "
                + "divisible by (xblocks,yblocks)"
            )
        elif (xblocks % 2 or yblocks % 2) and xblocks != 1 and yblocks != 1:
            raise ValueError(
                "[flowx.domain.GridBase]:(xblocks,yblocks) must be exactly "
                + "divisible by 2"
            )
        ...
        block_attributes = self.__class__.initialize_block_attributes(
            xblocks, yblocks, dx, dy, xmin, xmax, ymin, ymax, levels
        )

        # Initialize data attributes
        nblocks = len(block_attributes)
        data_attributes = self.__class__.initialize_data_attributes(
            nblocks, nxb, nyb, varlist
        )
    ...
    @staticmethod
    def initialize_block_attributes(
        xblocks, yblocks, dx, dy, xmin, xmax, ymin, ymax, levels
    ):
        ...
        for lblock in range(xblocks * yblocks):
            ...
            imin, imax = [
                xmin + (iloc / xblocks) * (xmax - xmin),
                xmin + ((iloc + 1) / xblocks) * (xmax - xmin),
            ]
            jmin, jmax = [
                ymin + (jloc / yblocks) * (ymax - ymin),
    ...
    def get_l2_norm(self, eror):
        ...
        for block in self.blocklist:
            l2_norm = l2_norm + (
                numpy.sqrt(numpy.sum(block[eror] ** 2))
    ...
    def fill_guard_cells(self, varlist, **kwargs):
        ...
        for varkey in varlist:
            ...
            for block in self.blocklist:
                ...
                for location, neighbor, delta, bc_type, bc_val in zip(
                    locations, neighbors, deltas, bc_type_var, bc_val_var
                ):
                    if neighbor is None:
                        if bc_type == "neumann":
                            self.__class__.fill_guard_cells_neumann(
                                blockdata, location, bc_val, delta
                            )
                        elif bc_type == "dirichlet":
                            self.__class__.fill_guard_cells_dirichlet(
                                blockdata, location, bc_val
                            )
                            ...
                            self.__class__.fill_guard_cells_projection(
                                blockdata, location
                            )
                        elif bc_type == None:
                            None
                            ...
                            raise ValueError(
                                'Boundary type "{}" not implemented'.format(bc_type)
                            )
