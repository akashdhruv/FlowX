...
def jit_solid_stress(sd,sX,sY,Tau1,Tau2,Tau3,Tau4,dx,dy,nx,ny):
    ...
    for i in range(1,nx-1):
        for j in range(1,ny-1):
            ...
            A_inv1 = 1./(2*dx)*(sX[i+1,j]-sX[i-1,j])
            A_inv2 = 1./(2*dy)*(sX[i,j+1]-sX[i,j-1])
            A_inv3 = 1./(2*dx)*(sY[i+1,j]-sY[i-1,j])
            A_inv4 = 1./(2*dy)*(sY[i,j+1]-sY[i,j-1])

            if(numpy.abs(A_inv1*A_inv4-A_inv2*A_inv3) > 1E-12):

                A1 = 1./(A_inv1*A_inv4-A_inv2*A_inv3)*A_inv4
                A2 = 1./(A_inv1*A_inv4-A_inv2*A_inv3)*(-A_inv2) 
                A3 = 1./(A_inv1*A_inv4-A_inv2*A_inv3)*(-A_inv3) 
                A4 = 1./(A_inv1*A_inv4-A_inv2*A_inv3)*A_inv1
